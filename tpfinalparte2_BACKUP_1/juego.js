class juego {
  ...
  crearPersonaje() {
    this.personaje = new personaje(width / 2, 465); // Crear el personaje
  }
  ...
  crearEnemigos() {
    ...
    for (let i = 0; i < 6; i++) {
      this.enemigos.push(new enemigo(600, i * 80 + 50, tiempos[i]));
    }
  }
  ...
  crearRecompensa() {
    this.recompensa = new recompensa(348, 72); // Colocar la recompensa en la posición inicial
  }
  ...
  reiniciarJuego() {
    ...
    this.personaje = new personaje(width / 2, 465); // Restablecer la posición del personaje
    this.enemigos = [];
    let tiempos = [5000, 3000, 7000, 2000, 4000, 6000];
    for (let i = 0; i < 6; i++) {
      this.enemigos.push(new enemigo(600, i * 80 + 50, tiempos[i]));
    }
    this.recompensa = new recompensa(348, 72); // Restablecer la recompensa
  }
